...
for org in database:
    if '.DS_Store' not in org:
        ...
        if args.syn_fa == 'Mauve':
            ...
            #Change path if you have enother
            p = pd.read_csv(args.out + '/{}_results_sibelia/blocks_coords.txt'.format(org), 
            header=None, 
                    ...
                    if refsta == stain[: -12]:
                        ...
                        for i in coord_g.index:
                            ...
                            if 0.2 * len(seq_i) < 10:
                                continue
                            left_edge = seq_i[: 10]
                            right_edge = seq_i[len(seq_i) - 10]
                            mid = seq_i[10: len(seq_i) - 10]
                            cake += ('>sb{}'.format(i))
                            cake += ('\n')
                            cake += (seq_i)
                            cake += ('\n')
                        ...
                        savefile = open(args.save_way + org + '/' + org + '_' + stain[: -12] + '.fasta', 'w')
                        savefile.write(cake)
                        savefile.close()
